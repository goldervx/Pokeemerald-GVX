...
static const struct WindowTemplate sDebugMenuWindowTemplateItemDescription =
{
    ...
    .baseBlock = 100, // En qué parte de VRAM se cargan tiles de ventana (Incluye marco y texto). Para calcularlo en mGba, herramientas->estado del juego->ver mosaicos, sección de bgs y ver qué parte de la memoria está vacía (en negro). Lo lógico es cargar las ventanas seguidas.
};
...
// *******************************
// Actions Give
#define ITEM_TAG 65011
#define tItemId    data[5]
#define tSpriteId  data[6]

static void Debug_Display_ItemInfo(u32 itemId, u32 digit, u8 subWindowId, u8 descWindowId)
{
    StringCopy(gStringVar2, gText_DigitIndicator[digit]);
    u8* end = CopyItemName(itemId, gStringVar1);
    WrapFontIdToFit(gStringVar1, end, DEBUG_MENU_FONT, WindowWidthPx(subWindowId));
    StringCopyPadded(gStringVar1, gStringVar1, CHAR_SPACE, 15);
    ...
    AddTextPrinterParameterized(subWindowId, DEBUG_MENU_FONT, gStringVar4, 0, 0, 0, NULL);
...
static void DebugAction_Give_Item(u8 taskId)
{
    ...
    windowId = AddWindow(&sDebugMenuWindowTemplateExtra);
    DrawStdWindowFrame(windowId, FALSE);
    windowItemDescription = AddWindow(&sDebugMenuWindowTemplateItemDescription);
    DrawStdWindowFrame(windowItemDescription, FALSE);

    CopyWindowToVram(windowId, COPYWIN_FULL);
    CopyWindowToVram(windowItemDescription, COPYWIN_FULL);

    // Display initial item
    Debug_Display_ItemInfo(1, 0, windowId, windowItemDescription);

    gTasks[taskId].func = DebugAction_Give_Item_SelectId;
    gTasks[taskId].tSubWindowId = windowId;
    gTasks[taskId].tWindowId = windowItemDescription;
...
static void DebugAction_Give_Item_SelectId(u8 taskId)
{
    if (JOY_NEW(DPAD_ANY))
    {
        ...
        Debug_Display_ItemInfo(
            gTasks[taskId].tInput,
            gTasks[taskId].tDigit,
            gTasks[taskId].tSubWindowId,
